# -------------------------------------------------------------------------------------------------
# spinup_eval.R
# Script to calculate rough spinup status for constant recharge runs
# -------------------------------------------------------------------------------------------------
...
source("~/mb_sensitivity/scripts/storagecalc.R")

# output files are not included, but can be generated
setwd("~/parflow_outputs/H_v0_outputs/")
...
mask <- readpfb("~/mb_sensitivity/scripts/supporting_files/A_v4.out.mask.pfb", verbose = F)

# reading entire time series of pressure files
press_files <- list.files(pattern="H_v0.out.press.*.pfb")
...
plot(storage,type="l",col="blue", main="Storage for Scenario H Spinup Run", tck =1, tcl = 0.5, ylab="storage (m^3)", xlab="time (x 10,000 hours)")

plot(rate_storage,type="l",col="green",log="y",ylab="Storage change divided by inflow (percent)",xlab="Time (x10,000 hours)",
     main=paste("Percentage storage change for Scenario H Spinup Run"), tck =1, tcl = 0.5)
